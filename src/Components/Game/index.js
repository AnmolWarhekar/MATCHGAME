...
class Game extends Component {
  ...
  getFilteredImages = () => {
    console.log('--> In getFilteredImages()')
  ...
  updateScoreAndGetNextImg = () => {
    ...
    console.log('--> In updateScoreAndGetNextImg()')
    const randomImgItem = this.getRandomImageItem()
    updateScore()
    this.setState({
      activeImgId: randomImgItem.id,
    })
  }
  ...
  render() {
    ...
    const activeImgItem = imagesList.find(
      eachItem => eachItem.id === activeImgId,
    )
    // console.log(`activeImgItem: ${activeImgItem}`)

    const activeImgUrl = activeImgItem.imageUrl
    const filteredImages = this.getFilteredImages()

    return (
      <div className="game-container">
        <div className="random-img-showing-container">
          <img src={activeImgUrl} alt="match" className="random-img" />
        </div>
